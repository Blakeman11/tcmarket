...
// src/app/progressive/[id]/page.tsx
import { prisma } from "@/lib/prisma"
import { notFound } from "next/navigation"
import Image from "next/image"

export default async function ProgressivePage({ params }: { params: { id: string } }) {
  const card = await prisma.cardListing.findUnique({
    where: { id: params.id },
  });

  if (!card || card.listingType !== "Progressive") return notFound();
    ...
    <main className="max-w-4xl mx-auto p-6">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold mb-2">{card.title}</h1>
          <p className="text-gray-600 mb-2">{card.playerName} - {card.year} - {card.brand}</p>
          <p className="font-semibold mb-4">Current Price: ${card.price.toFixed(2)}</p>

          <form action="/api/progressive-bid" method="POST" className="space-y-4">
            <input type="hidden" name="listingId" value={card.id} />
            <input
              type="number"
              step="0.01"
